...
class ImageDataset(Dataset):
    ...
    def __init__(self, data, targets, transform=None, is_change_target=False):
        super(ImageDataset, self).__init__()
        self.data = data
        self.targets = targets
        self.transform = transform
        self.is_change_target = is_change_target
...
class ImageDatasetModule(object):
    ...
    def split_dataset(self, data_list, target_list):
        ...
        num_ood_client = self.params['num_ood_client'] # 추가됨. 3, 5, 7
        ...
        num_ood_clients = math.ceil(len(id_targets)/num_id_target_per_client) # 기존. 여기서는 5로 설정됨. 3, 7일 경우 elif 문에서 업데이트됨

        #######################################
        # abnormal 포함하는 client 개수 나누는 부분 #
        #######################################
        assert num_ood_client in [2,3,4,5,6,7], 'num_ood_client not ok'
        ...
        for comb_idx, idx_list in enumerate(np.array(class_comb)):
            ...
            ##################################### static 하게 할당
        if num_ood_client == 5: # [array([0, 2]), array([3, 4]), array([5, 6]), array([7, 8]), array([8, 9])]
            pass

        elif num_ood_client == 2: # [array([0, 2]), array([3, 4]) ]
            normal_client_class_idx_list.append(abnormal_client_class_idx_list.pop(2))
            normal_client_class_idx_list.append(abnormal_client_class_idx_list.pop(2))
            normal_client_class_idx_list.append(abnormal_client_class_idx_list.pop(2))
            num_ood_clients = num_ood_client # update

        elif num_ood_client == 3: # [array([0, 2]), array([3, 4]), array([5, 6]) ] 
            normal_client_class_idx_list.append(abnormal_client_class_idx_list.pop(3))
            normal_client_class_idx_list.append(abnormal_client_class_idx_list.pop(3))
            num_ood_clients = num_ood_client # update
            
        elif num_ood_client == 4: # [array([0, 2]), array([3, 4]), array([5, 6]), array([8, 9])]
            normal_client_class_idx_list.append(abnormal_client_class_idx_list.pop(3))
            num_ood_clients = num_ood_client # update

        elif num_ood_client == 6: # [array([0, 2]), array([3, 4]), array([5, 6]), array([7, 8]), array([8, 9]), array([0, 3])]
            abnormal_client_class_idx_list.append(normal_client_class_idx_list.pop(0))
            num_ood_clients = num_ood_client # update

        elif num_ood_client == 7: # [array([0, 2]), array([3, 4]), array([5, 6]), array([7, 8]), array([8, 9]), array([0, 3]), array([7, 9])]
            abnormal_client_class_idx_list.append(normal_client_class_idx_list.pop(0))
            abnormal_client_class_idx_list.append(normal_client_class_idx_list.pop(-1))
            num_ood_clients = num_ood_client # update
            ######################################
        ...
        print(f'num_clients : {num_clients}, num_abnormal_clients : {num_abnormal_clients}, num_normal_clients : {num_normal_clients} ')
        print(f'abnormal_clients : {abnormal_client_class_idx_list}')
        ...
        print('portion: ', round(data_portion * dataset.__len__()))

        dataset = Subset(dataset, list(range(0, round(data_portion * dataset.__len__()))))
        labels = Subset(labels, list(range(0, round(data_portion * labels.__len__()))))
    ...
    def convert_dataset(self, data_dict, client_idx, is_change_target=False):
        ...
        return dict(
            train_dataset=ImageDataset(data=train_X, targets=train_y, transform=transform, is_change_target=is_change_target),
            val_dataset=ImageDataset(data=valid_X, targets=valid_y, transform=transform, is_change_target=is_change_target),
            test_dataset=ImageDataset(data=test_X, targets=test_y, transform=transform, is_change_target=is_change_target),
        )

    def convert_total_dataset(self, data_dict, is_change_target=False):
        ...
        return ImageDataset(data=test_X, targets=test_y, transform=transform, is_change_target=is_change_target)
