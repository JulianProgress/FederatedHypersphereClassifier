...
def weight_to_state_dict(keys, weights, save=False, file_name=''):
    # state_dict = OrderedDict({k: torch.tensor(v) for k, v in zip(keys, weights)})
    # return state_dict
    dic = OrderedDict()
    for k, v in zip(keys, weights):
        if 'num_batches_tracked' in k:
            dic[k] = torch.from_numpy(np.array([v]))
            # print(k, dic[k])
        else:
            dic[k] = torch.from_numpy(v)
            # print(dic[k])

    # print('Dict: ', dic)
    if save:
        torch.save(dic, f'../tmp/tmp{file_name}.pt')
    return dic
...
class RHSCFedTrainingf(ABC):
    def __init__(self, num_clients: int, num_rounds: int, eval_rounds: int, trainer_params, model_params,
        ...
        super(RHSCFedTrainingf, self).__init__()
    ...
    def round(self, rnd, trainset, validset, testset, host_list, is_aug):
        ...
        # print(global_center, global_r, len(ts), len(vs), hl, abnormal_list)
...
def evaluation(args):
    ...
    num_test, results, test_auc, acc, return_type = trainer.test(testl, net, r=r)  # return type = 1: normal acc
...
def RHSC_client(args):
    ...
    print(f'hostid : {hostid}, device: {device}, {model_params}')

    net = SVDD(**model_params)
    keys = net.state_dict().keys()
    dic = weight_to_state_dict(keys, g_weights, save=True, file_name=hostid)

    # state_dict = torch.load(f'../tmp/tmp{hostid}.pt')
    print('loaded done')
    net = SVDD(**model_params)
    print('reinit done')
    ...
    # print('net keys done')
    #
    # # print(len(keys), len(g_weights))
    # # params_dict = zip(keys, g_weights)
    #
    # # for key in keys:
    # #     print(key)
    #
    ...
    #     # print(od)
    #
    # g_state_dict = weight_to_state_dict(keys, g_weights)
    # print('net states done')
    #
    # # print(params_dict)
    # # state_dict =
    # # print(state_dict)
    # net.load_state_dict(g_state_dict)  # load global state dict
    # print('loaded not done')
