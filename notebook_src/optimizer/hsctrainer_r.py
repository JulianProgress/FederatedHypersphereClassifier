    ...
    def train(self, dataset: DataLoader, validset: DataLoader, net: BaseNet):
        ...
        for epoch in range(self.n_epochs):
            ...
            for i, data in enumerate(dataloader):
                ...
                abnormal_idx_list = y.type(torch.bool)
                total_n = len(y)
                abnormal_n = len(abnormal_idx_list)
                normal_n = total_n - abnormal_n
                ...
                # losses = (1 - y) * torch.log((dist - self.R) ** 2) + y * torch.log(
                #     1 - ((dist - self.R) ** 2)) + self.pert_gamma * torch.abs(self.R)
                # losses = (1 - y) * (dist - self.R) - y * torch.log(
                #     1 - torch.exp(-(dist - self.R))) + self.pert_gamma * torch.abs(self.R)
                losses = ((1 - y) * ((dist - self.R) ** 2) - y * torch.log(1-torch.exp(-((dist - self.R) ** 2)))) + self.pert_gamma * torch.abs(self.R)
                # losses = ((1 - y) * torch.abs(dist - self.R) - y * torch.log(
                #     1 - torch.exp(-torch.abs(dist - self.R))))  # + self.pert_gamma * torch.abs(self.R)
                ...
                # r_loss = (1 - y) * (dist.data - self.R) - y * torch.log(
                #     1 - torch.exp(-(dist.data - self.R))) + self.pert_gamma * torch.abs(self.R)
            ...
            for data in validset:
                ...
                #     1 - ((dist - self.R) ** 2)) + self.pert_gamma * torch.abs(self.R)
                # val_losses = (1 - y) * (dist - self.R) - y * torch.log(
                #     1 - torch.exp(-(dist - self.R))) + self.pert_gamma * torch.abs(self.R)
            ...
            if valid_loss.avg < best_loss and epoch > 10:
                if self.verbose:
