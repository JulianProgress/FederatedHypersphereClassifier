...
class DSVDDTrainer(BaseTrainer):
    def __init__(self, optimizer_name: str, lr: float, n_epochs: int, lr_milestones: tuple,
                 weight_decay: float, device: str, nu: float, random_seed, pretrain_lr, pretrain_weight_decay):
    ...
    def train(self, data_loader, valid_loader, net: RNNAE = None, isSVDD=True, T=1, patience=5):
        ...
        for epoch in tqdm(range(self.n_epochs)):
            ...
            message = 'Epoch: {0} \t| Train Loss: {1:.8f}, Validation Loss: {2:.8f}'.format(epoch + 1, svdd_loss.avg,
                                                                                            val_ae_loss.avg)
            print(message)
