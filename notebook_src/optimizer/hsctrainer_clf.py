...
class RHSCTrainer(BaseTrainer):
    ...
    def test(self, dataset: DataLoader, net: BaseNet, c=None, r=None):
        ...
        if labels.sum() > 0:
            ...
            pred_list = np.zeros_like(scores)
            pred_list[scores > best_thresh] = 1

            obj = classification_report(labels, pred_list, output_dict=True)
    ...
    def test_threshold(self, val_dataset: DataLoader, test_dataset: DataLoader, net: BaseNet):
        ...
        test_pred_list = np.zeros_like(test_score_list)
        test_pred_list[test_score_list > best_thresh] = 1

        test_result = classification_report(test_label_list, test_pred_list, output_dict=True)
