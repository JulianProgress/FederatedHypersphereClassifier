    ...
    def train(self, dataset: DataLoader, validset: DataLoader, net: BaseNet):
        ...
        for epoch in range(self.n_epochs):
            ...
            for data in validset:
                ...
                val_loss =  (normal_len / total_len) * (1 - y) * ((dist - self.R) ** 2) -  ((abnormal_len / total_len) * y * torch.log(
                    1 - torch.exp(-((dist - self.R) ** 2)))) + self.lamb * torch.abs(self.R)
                ...
                score_list = concatenate(score_list, val_loss.detach().cpu().numpy())
            ...
            print("%s Epoch %d \t| training dist: %.4f, training loss: %.8f, validation auc: %.8f, R: %.8f" % (
