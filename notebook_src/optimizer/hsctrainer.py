...
class HSCTrainer(BaseTrainer):
    ...
    def train(self, dataset: DataLoader, validset: DataLoader, net: BaseNet):
        ...
        for epoch in range(self.n_epochs):
            ...
            for i, data in enumerate(dataloader):
                ...
                dist = torch.sqrt(torch.sum((svdd_out) ** 2, dim=1))
            ...
            if valid_loss.avg < best_loss and epoch >= 5:
                if self.verbose:
        ...
        return best_net, num_examples_train
    ...
    def test(self, dataset: DataLoader, net: BaseNet, c=None, global_thresh=None):
        ...
        with torch.no_grad():
            for X, y in dataset:
                ...
                dist = torch.sqrt(torch.sum(outputs ** 2, dim=1))
                # print('test3')
    ...
    def test_threshold(self, val_dataset: DataLoader, test_dataset: DataLoader, net: BaseNet):
        ...
        for data in val_dataset:
            ...
            probs = torch.sqrt(torch.sum((outputs) ** 2, dim=1))
        ...
        return tpr, fpr, test_auc, test_result, best_thresh