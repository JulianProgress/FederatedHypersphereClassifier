...
from datasets.image_v8_Copy import MNISTDatasetModule, CIFAR10DatasetModule
...
class RHSCFedTraining(ABC):
    def __init__(self, loger, num_clients, num_abnormal_clients, num_normal_clients, num_rounds: int,
                 ...
                 rms, rg, rlr, clr, abnormal_client_list=None, file_name='mnist', model_name='RHSC', abnormal_in_val=False,
    ...
    def lr_decay(self, rnd):
        if rnd % self.rms == 0 and rnd != 0:
            self.rlr *= self.rg
            self.lr *= self.rg
            self.descriptor_params['r_lr'] = self.rlr
            self.descriptor_params['lr'] = self.lr
            print(f"update learning rate: {self.rlr}, {self.lr}")
    ...
    def get_save_path(self, rnd, file_name):
        self.save_dir_path = self.trainer_params[
            "model_save_path"] if "model_save_path" in self.trainer_params else f"/workspace/code/FedHSC/model_save/{self.save_path}"
    ...
    def fit(self, trainset, validset, testset, host_list):
        ...
        for rnd in range(self.num_rounds):
            ...
            # if best_f1 < f1_mean:
            #     best_f1 = f1_mean
            #     shutil.copy(self.get_save_path(rnd, self.file_name), f'/workspace/code/FedHSC/model_save/best_models/{best_file}_bestf1.pt')
            #
            # if best_auc < auc_mean:
            #     best_auc = auc_mean
            #     shutil.copy(self.get_save_path(rnd, self.file_name),
            #                 f'/workspace/code/FedHSC/model_save/best_models/{best_file}_bestauc.pt')
            #
            # if best_acc < acc_mean:
            #     best_acc = acc_mean
            #     shutil.copy(self.get_save_path(rnd, self.file_name),
            #                 f'/workspace/code/FedHSC/model_save/best_models/{best_file}_bestacc.pt')
...
def multi_eval(args):
    ...
    if is_abnormal:
        ...
        num_test, obj, test_auc, acc, return_type = trainer.test(testset, net, trainer_params, r=r, c=torch.tensor(c).to(device))  # return type = 1: normal acc
...
def evaluation(args):
    ...
    num_test, results, test_auc, acc, return_type = trainer.test(testset, net, trainer_params, c=c, r=r)  # return type = 1: normal acc
...
def RHSC_client(args):
    ...
    c, R, net, num_train = trainer.train(trainset, validset, net, trainer_params)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--r_milestone', '-rms', type=int, default=40, help='milestone for learning rate decay')
    parser.add_argument('--r_gamma', '-rg', type=float, default=0.5, help='r gamma for learning rate decay')
    ...
    parser.add_argument('--num_ood_client', '-no', type=int, default=5, help='number of ood client')
    ...
    if model_type == 'cifar10': # or model_type == 'toniot' or model_type == 'cicids'
        rep_dim = 128
    else:
        rep_dim = 32
    ...
    if model_type == 'mnist' or model_type == 'cifar10':
        ...
        dataset_params = dict(
            ...
            random_seed=999,
            num_ood_client=args.num_ood_client
        )
        ...
        # data = load_data('../../data/cifar10_dataset.pkl')
        # train_set = data['trainset']
        # valid_set = data['validset']
        # test_set = data['total_test_dataset']
        # 
        # trainset = [TensorDataset(torch.from_numpy(ts[0]).float(), torch.from_numpy(ts[1]).int()) for ts in train_set]
        # validset = [TensorDataset(torch.from_numpy(ts[0]).float(), torch.from_numpy(ts[1]).int()) for ts in valid_set]
        # total_test_dataset = TensorDataset(torch.from_numpy(test_set[0]).float(), torch.from_numpy(test_set[1]).int())
        # 
        # num_clients = data['num_clients']
        # abnormal_clients = data['abnormal_clients']
        # normal_clients = data['normal_clients']

    else:

        data_portion = args.data_portion

        if model_type == 'cicids':
            folder_name = 'CICIDS2017'
        else:
            folder_name = 'ton'

        data_path = f'/workspace/dataset/data/{folder_name}/data_dict_folder'
        print(data_path)

        info = load_data(os.path.join(data_path, 'information.pkl'))
        num_clients = info['num_clients']  # 36
        abnormal_clients = info['num_abnormal_clients']  # 5
        normal_clients = info['num_normal_clients']  # 31

        client_train_X = np.load(os.path.join(data_path, 'client_train_X.npy'), allow_pickle=True)
        client_train_y = np.load(os.path.join(data_path, 'client_train_y.npy'), allow_pickle=True)
        # client_test_X = np.load(os.path.join(data_path, 'client_test_X.npy'), allow_pickle=True)
        # client_test_y = np.load(os.path.join(data_path, 'client_test_y.npy'), allow_pickle=True)
        client_valid_X = np.load(os.path.join(data_path, 'client_valid_X.npy'), allow_pickle=True)
        client_valid_y = np.load(os.path.join(data_path, 'client_valid_y.npy'), allow_pickle=True)

        trainset = []
        validset = []

        for client in range(num_clients):
            train_set = TensorDataset(torch.from_numpy(client_train_X[client][:round( len(client_train_X[client]) * data_portion ) ]).float(),
                                      torch.from_numpy(client_train_y[client][:round( len(client_train_y[client]) * data_portion ) ]).int())
            valid_set = TensorDataset(torch.from_numpy(client_valid_X[client][:round( len(client_valid_X[client]) * data_portion ) ]).float(),
                                      torch.from_numpy(client_valid_y[client][:round( len(client_valid_y[client]) * data_portion ) ]).int())
            trainset.append(train_set)
            validset.append(valid_set)

        test_all_X = np.load(os.path.join(data_path, 'test_all_X.npy'))
        test_all_y = np.load(os.path.join(data_path, 'test_all_y.npy'))

        total_test_dataset = TensorDataset(torch.from_numpy(test_all_X[:round( len(test_all_X) * data_portion ) ]).float(),
                                      torch.from_numpy(test_all_y[:round( len(test_all_y) * data_portion ) ]).int())
    ...
    fl_trainer = RHSCFedTraining(loger,
                                 rms=args.r_milestone,
                                 rg=args.r_gamma,
                                 rlr=args.r_learning_rate,
                                 clr=args.c_learning_rate,
