...
class ImageDatasetModule(object):
    ...
    def split_dataset(self, data_list, target_list):
        ...
        data_dict = {target: dict(entire_data_list=list(), entire_target_list=list()) for target in set(target_list.numpy())}
        client_dict = {
            client: dict(
        ...
        idx_list = np.concatenate([np.random.permutation(id_targets) for _ in range((num_clients * num_id_target_per_client) // len(id_targets) + 1)])
    ...
    def convert_total_dataset(self, client_dict):
        ...
        return dict(
            train_dataset=ImageDataset(data=total_train_data_list, targets=total_train_target_list, transform=transform),
