...
class RHSCTrainer(BaseTrainer):
    def __init__(self, optimizer_name: str, lr: float, n_epochs: int, lr_milestones: tuple,
                 ...
                 include_pert: bool, gamma: float,
                 radius_thresh: float, pert_steps: int, pert_step_size: float, pert_duration: int, client_wise=False,
                 update_c=False, lamb: float=0.5):
    ...
    def init_radius(self, net, dataloader):
        ...
        self.R = torch.tensor([0.], requires_grad=True, device=self.device)
        ...
        for epoch in range(self.n_epochs):
            ...
            for i, data in enumerate(dataloader):
                ...
                losses = ((1 - y) * ((dist - self.R) ** 2) - y * torch.log(
                    1 - torch.exp(-((dist - self.R) ** 2)))) + self.pert_gamma * torch.abs(self.R)
                ...
                r_loss = ((1 - y) * ((dist.data - self.R) ** 2) - y * torch.log(
                    1 - torch.exp(-((dist.data - self.R) ** 2)))) + self.pert_gamma * torch.abs(self.R)
            ...
            for data in validset:
                ...
                val_losses = ((1 - y) * ((dist - self.R) ** 2) - y * torch.log(
                    1 - torch.exp(-((dist - self.R) ** 2)))) + self.pert_gamma * torch.abs(self.R)
