...
class RHSCTrainerV3(BaseTrainer):
    def __init__(self, optimizer_name: str, lr: float, n_epochs: int, lr_milestones: tuple,
        ...
        super(RHSCTrainerV3, self).__init__(optimizer_name, lr, n_epochs, lr_milestones, weight_decay, device)
        self.r_lr = r_lr
        self.c_lr = c_lr
    ...
    def train(self, trainset, validset, net: BaseNet, trainer_params):

        if trainset.__len__() < trainer_params['trainloader']['batch_size']:
            trainer_params['trainloader']['batch_size'] = trainset.__len__()
        if validset.__len__() < trainer_params['valloader']['batch_size']:
            trainer_params['valloader']['batch_size'] = validset.__len__()

        trainloader = DataLoader(trainset, **trainer_params['trainloader'])
        validloader = DataLoader(validset, **trainer_params['valloader'])

        # print(self.verbose)
        # if self.c_idx == 'client3':
        ...
        if self.c is None:
            if self.verbose:
                print('init c..')
            self.initialize_c(net, trainloader)
            # print(self.c.data)
        # radius = self.get_radius(net, dataset)
        if self.R is None:
            print('init R..')
            self.init_radius(net, trainloader)
        ...
        for epoch in range(self.n_epochs):
            ...
            if self.include_pert and (pert_bool or (epoch + 1) % self.pert_duration == 0):
                ...
                self.low_conf_X, radiuses = self.get_low_confidence(net, trainloader)
            ...
            if epoch > self.init_steps and self.include_pert:
                dataloader = zip(trainloader, self.pert_loader)
            else:
            ...
            for i, data in enumerate(trainloader):
                ...
                # # losses = (normal_len / total_len) * ((1 - y) * ((dist - self.R) ** 2)) - (abnormal_len / total_len) * (
                # #             y * torch.log(1 - torch.exp(-((dist - self.R) ** 2)))) + self.lamb * torch.abs(self.R)
                # losses = (normal_len / total_len)*((1 - y) * ((dist - self.R))) - (abnormal_len / total_len) * (y * torch.log(
                ...
                # # if self.is_abnormal:
                # self.R.requires_grad = True
                # r_loss = (normal_len / total_len) * ((1 - y) * ((dist.data - self.R))) - (abnormal_len / total_len) * (y * torch.log(
                ...
                self.R.requires_grad = False
                # (abnormal_len / total_len) *
                losses = (normal_len / total_len) * (1 - y) * ((dist - self.R) ** 2) - (
                            (abnormal_len / total_len) * y * torch.log(
                        1 - torch.exp(-((dist - self.R) ** 2)))) + self.lamb * torch.abs(self.R)

                # losses_v2 = (1 - y) * (torch.abs(dist - (self.R))) - y * torch.log(
                #     1 - torch.exp(-(dist - (self.R)))
                # ) + self.lamb * torch.abs(self.R)
                ...
                # if self.is_abnormal:
                self.R.requires_grad = True
                r_loss = ((normal_len / total_len) * (1 - y) * ((dist.data - self.R) ** 2)) - (
                            (abnormal_len / total_len) * y * torch.log(
                        1 - torch.exp(-((dist.data - self.R) ** 2)))) + self.lamb * torch.abs(self.R)
            ...
            #     mr = self.mean_radius(net, dataloader)
            #     self.R.data = torch.tensor(mr).to(self.device)
            ...
            for data in validloader:
                ...
                # val_loss = (normal_len / total_len) * (1 - y) * ((dist - self.R) ** 2) - (
                #             (abnormal_len / total_len) * y * torch.log(
                #         1 - torch.exp(-((dist - self.R) ** 2)))) + self.lamb * torch.abs(self.R)
                #
                ...
                # val_losses = ((1 - y) * dist - y * torch.log(1-torch.exp(-dist)))
                # if self.val_set == 0:
                #     val_losses = (normal_len / total_len) * ((1 - y) * ((dist - self.R) ** 2)) - (
                #                 abnormal_len / total_len) * (y * torch.log(
                #         1 - torch.exp(-((dist - self.R) ** 2)))) + self.lamb * torch.abs(self.R)
                #
                #     # ((1 - y) * ((dist - self.R) ** 2) - y * torch.log(
            ...
            if self.val_set == 2:
                self.initialize_c(net, trainloader)  # update c
            if self.verbose:
                if self.is_abnormal:
    ...
    def test(self, dataset, net: BaseNet, trainer_params, c=None, r=None):

        testl = DataLoader(dataset, **trainer_params['testloader'])
        start_time = time.time()
        net.to(self.device)
        net.eval()
