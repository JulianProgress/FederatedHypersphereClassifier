...
class FederatedTraining(ABC):
    def __init__(self, num_clients: int, num_rounds: int, trainer_params, model_params, descriptor_params,
                 cuda_num_list, model_name='RHSC', abnormal_in_val=False, rep_model='RNNAE', eval_device='cuda:0', personalization=True):
    ...
    def get_save_path(self, rnd):
        ...
        return os.path.join(self.save_dir_path,
                            f"{self.model_name}_rep{self.rep_model}_ps{self.descriptor_params['pert_steps']}_pd{self.descriptor_params['pert_duration']}_aug{int(self.descriptor_params['include_pert'])}_l{self.model_params['num_rep_layer']}_s{self.model_params['num_svdd_layer']}_{rnd}.pt")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--radius_thresh', '-rt', type=float, default=0.95, help='threshold for local radius calculation')
    ...
    fl_trainer = FederatedTraining(num_clients=args.num_clients,
    ...
    fl_trainer.fit()
    local_auc, local_f1, local_acc = fl_trainer.eval()

    save_data(dict(
        auc=local_auc,
        f1=local_f1,
        acc=local_acc
    ), '/workspace/code/FedHSC/results', f"Fed_aug{int(descriptor_params['include_pert'])}_result.pkl")
