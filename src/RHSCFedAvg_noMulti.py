...
from datasets import ADDRepDataset, MNISTDataset
...
class RHSCFedNoMulti(ABC):
    def __init__(self, num_clients: int, num_rounds: int, eval_rounds: int, trainer_params, model_params,
                 descriptor_params,
                 cuda_num_list, model_name='RHSC', abnormal_in_val=False, save_path=None, rep_model='RNNAE',
                 eval_device='cuda:0',
                 personalization=True):
        super(RHSCFedNoMulti, self).__init__()
    ...
    def get_save_path(self, rnd):
        ...
        return os.path.join(self.save_dir_path,
                            f"mnist_{self.model_name}_gamma{self.descriptor_params['gamma']}_{rnd}.pt")
...
def fit(self, test_portion):
    ...
    dataset = MNISTDataset(test_portion, 0.2, abnormal_in_val=self.abnormal_in_val)
    ...
    for rnd in range(self.num_rounds):
        ...
        if self.current_rnd % self.eval_rounds == 0:
            test_num, obj, test_auc, return_type = self.eval(test_portion)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', '-b', type=int, default=32, help='batch size for local update')
    ...
    parser.add_argument('--cuda_num', '-c', type=int, default=0, help='cuda num')
    ...
    model_params = {
        ...
        'in_dim': 32,
        'hidden_dims': [32],  # hidden_dim, rep_dim
        'window_size': 128,
        'num_svdd_layer': 3,
        'num_rep_layer': 3
    }
    ...
    fl_trainer = RHSCFedNoMulti(save_path=save_path,
                                 ...
                                 cuda_num_list=[args.cuda_num for _ in range(args.num_clients)],
