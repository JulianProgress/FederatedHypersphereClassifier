...
from datasets.image_v8 import MNISTDatasetModule, CIFAR10DatasetModule
...
if __name__ == '__main__':
    ...
    parser.add_argument('--verbose', '-ver', dest='verbose', action='store_true')
    ...
    parser.add_argument('--num_client', '-nc', type=int, default=3, help='number of client')
    ...
    if args.additional != '':
        file_name = f"{args.mode}_{args.additional}_{args.num_epochs}_tr{args.train_ratio}_nc{args.num_client}"
        print(file_name)
    ...
    print(trainer_params['model_save_path'])
    ...
    descriptor_params = {'optimizer_name': 'Adam', 'lr': args.learning_rate, 'n_epochs': args.num_epochs,
                         'lr_milestones': [20, 40], 'verbose': args.verbose, 'return_best': return_best,
    ...
    if model_type == 'mnist' or model_type == 'cifar10':
        ...
        BASE_PATH = f'/workspace/dataset/data/{folder_name}/cicids_final_portion30'
        print(BASE_PATH)
        #
        # data_path = f'/workspace/dataset/data/{folder_name}/data_dict_folder'
        # print(data_path)

        data_file = f'{BASE_PATH}/cicids_client{num_client}_trainratio_{train_ratio}.pickle'
        res = load_data(data_file)

        num_clients = res['num_clients']  # 15
        abnormal_clients = num_client# res['num_abnormal_clients']  # 5
        normal_clients = num_clients - abnormal_clients#res['num_normal_clients']  # 31
        ...
        for client in range(res['num_clients']):
            ...
            test_set = TensorDataset(torch.from_numpy(data_dict[client]['test_X']).float(),
                                     torch.from_numpy(data_dict[client]['test_y']).int())
